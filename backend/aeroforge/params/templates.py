...
_NOTE_FILL = (
    "有效加注比例 = 公开推进剂质量 / 几何满箱质量（按 §5.9 九段分区装配账满箱口径："
    "箱段柱体 = 级长 − 发动机高 − 分区预留〔两端封头 + 级间舱/隔板段〕，另计封头容积；"
    "ρ 按推进剂物性表。2026-09-20 按新装配账满箱口径重标——装配树与 §8.4 账同源化后"
    "满箱容积较旧口径缩小，fill 相应上调；它是固定火箭质量账命中公开分项的标定参数，"
    "性能评估链的推进剂账入口，§8.7）"
)
...
FALCON9_SOURCED_FIELDS: dict[str, str] = {
    ...
    "stages[0].engine_height_m": (
        "工程惯例估算（Merlin 1D 海平面型净安装高，安装基准面取机架安装面；公开资料未统一）。"
        "2026-09-20 口径修正：原 3.0 m 混入了机架/推力结构高度，而 §5.9 分区已单列"
        "「推力结构」段——本字段取纯发动机占高 2.4 m 避免双计，并与公开推进剂 411 t "
        "在 42.6 m 级长下的满箱相容性复核一致"
    ),
...
#: 有效加注比例（公开加注量 / 几何满箱质量，§5.9 九段分区装配账满箱口径 → §13.2 公开
#: 分项；2026-09-20 按新装配账口径重标）：
#: S-IC 满箱 2,671,491 kg（级长 42.1 − 发动机 5.6 − 分区预留 10.1〔2:1 封头 ×2 + 级间舱〕
#: → 箱段柱体 26.4 m + 4 封头；O/F 2.27）→ 2,149,500/2,671,491 ≈ 0.8046；
#: S-II（共底）满箱 689,402 kg → 443,000/689,402 ≈ 0.6426；
#: S-IVB（共底）满箱 196,280 kg → 106,600/196,280 ≈ 0.5431。
_SV_SIC_FILL = 0.8046
_SV_SII_FILL = 0.6426
_SV_SIVB_FILL = 0.5431
...
#: 有效加注比例（公开加注量 / 几何满箱质量，§5.9 九段分区装配账满箱口径 → §13.2 公开
#: 分项；2026-09-20 按新装配账口径重标）：
#: 芯一级满箱 185,128 kg（级长 31 − 发动机 3.2 − 分区预留 5.0〔2:1 封头 ×2 + 级间舱〕
#: → 箱段柱体 22.8 m + 4 封头；O/F 5.0）→ 158,000/185,128 ≈ 0.8535；
#: 芯二级满箱 74,783 kg（O/F 5.5）→ 23,000/74,783 ≈ 0.3075
#: （真实芯二级大部分级长为发动机舱与级间段，满箱远大于公开加注量）；
#: 助推器满箱 199,380 kg（O/F 2.6）→ 145,000/199,380 ≈ 0.7272。
_CZ5_CORE1_FILL = 0.8535
_CZ5_CORE2_FILL = 0.3075
_CZ5_BOOSTER_FILL = 0.7272
